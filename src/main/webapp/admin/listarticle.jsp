...
<body>
	<nav class="breadcrumb">
		...
		网站内容列表 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px"
	...
	<div class="page-container">
		...
		<form action="article/deleteArticleByIds.action" method="post" id="myform" name="myform">
			<table class="table table-border table-bordered table-bg">
				<thead>
					<tr>
						<th scope="col" colspan="10">网站内容列表</th>
					</tr>
					<tr class="text-c">
				...
				<c:forEach items="${articleList}" var="article">
					<tr class="text-c">
						<td><input type="checkbox" value="${article.articleid}" name="articleid"></td>
						<td>${article.title}</td>
						<td>${article.bannername}</td>
						<td>${article.istop}</td>
						<td>${article.isflv}</td>
						<td>${article.addtime}</td>
						<td>${article.hits}</td>
						<td><a href="article/getArticleById.action?id=${article.articleid}"><i class="Hui-iconfont">&#xe6df;</i></a>&nbsp;&nbsp;<a
