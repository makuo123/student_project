...
<body>
	<nav class="breadcrumb">
		...
		医生排班查询 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px"
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="6">医生排班查询</th>
				</tr>
				<tr class="text-c">
			...
			<c:forEach items="${worksList}" var="works">
				<tr class="text-c">
					<td class="center">${works.wno}</td>
					<td class="center">${works.deptname}</td>
					<td class="center">${works.doctorname}</td>
					<td class="center">${works.workdate}</td>
					<td class="center">${works.memo}</td>
				</tr>
			</c:forEach>
		</table>
		<div class="cl pd-5 mt-20"></div>
		<div class="text-c">
			<form action="works/queryWorksByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						<option value="wno">按排班号查询</option>
						<option value="deptid">按科室查询</option>
						<option value="doctorid">按医生查询</option>
						<option value="workdate">按日期查询</option>
